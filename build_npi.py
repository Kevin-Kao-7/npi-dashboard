...
def detect_mp_changes(new_records):
    """对比上次构建的 JSON，检测 MP 日期或 stage 有变动的记录，标记 mp_changed"""
    ...
    try:
        ...
        old_records = {r["orig_idx"]: r for r in old_data.get("records", [])}
    except Exception:
        return new_records

    for r in new_records:
        idx = r["orig_idx"]
        old = old_records.get(idx)
        if not old:
            # 新增记录，不算 MP 变动
            continue
...
// ── 渲染列表 ──
function renderList(){{
  const hasFilter=filterModelVals.size>0||filterMktVals.size>0||searchModelVal||searchMktVal;
  ...
  sorted.forEach(r=>{{
    ...
    h+='<tr onclick="openDetail('+r.orig_idx+')">';
